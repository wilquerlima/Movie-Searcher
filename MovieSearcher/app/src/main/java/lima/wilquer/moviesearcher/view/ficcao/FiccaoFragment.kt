...
class FiccaoFragment : Fragment(), BaseContract.View {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view : View = inflater.inflate(R.layout.fragment_ficcao, container, false)
        rv = view.recycleViewFiccao
        return view
    }
    ...
    override fun showMovies(movies: List<Movies>?) {
        ...
        for (i in 0 until movies!!.size - 1) {
            for (j in 0 until movies[i].genre_ids.size - 1)
                if (Constants.ID_GENRE_FICCAO == movies[i].genre_ids[j]) {
    ...
    companion object {
        ...
        fun newInstance(): FiccaoFragment {
            return FiccaoFragment()
        }
    }
}