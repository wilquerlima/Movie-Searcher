package lima.wilquer.moviesearcher.view.fantasia
...
class FantasiaFragment : Fragment(), BaseContract.View {
    ...
    override fun showMovies(movies: List<Movies>?) {
        ...
        for (i in 0 until movies!!.size - 1) {
            for (j in 0 until movies[i].genre_ids.size - 1)
                if (Constants.ID_GENRE_FANTASIA == movies[i].genre_ids[j]) {
    ...
    companion object {
        ...
        fun newInstance(): FantasiaFragment {
            return FantasiaFragment()
        }
    }
}